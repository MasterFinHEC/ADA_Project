...
class data:
    ...
    def split(self, percentSplit):
        self.NumSplit = int(np.round(percentSplit * len(self.y)))
        self.Split = np.random.permutation(self.y.index)
        self.TrainSplit = self.Split[0:self.NumSplit]
        self.TestSplit = self.Split[self.NumSplit:len(self.y)]
